...
const universities = graduates.map((person) => {return person.university}).sort();
...
const contactInfo = graduates.map((person) => {return `${person.first_name} ${person.email}`});
...
graduates.forEach((person) => {
  if (person.university.includes('Uni')) { unisWithUni.push(person) };
});
...
const lowCaseAnimalNames = zooAnimals.map((animal) => {return animal.animal_name.toLowerCase()});
...
*/
const lowPopulationAnimals = zooAnimals.filter((animal) => {return animal.population < 5});
...
*/
const populationTotal = zooAnimals.reduce((total, animal) => {return total + animal.population}, 0);
